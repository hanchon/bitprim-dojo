...
// This doesn' t work because the cmakelist file forces the use of cpp 11
...
protected:
  ...
  public:
    Session(std::size_t max_response_streambuf_size,
        ...
        : connection(std::move(connection)), request_streambuf(std::move(request_streambuf)),
...
public:
  ...
  void request(const std::string &method,
    ...
    auto request_callback = std::make_shared<std::function<void(std::shared_ptr<Response>,
...
protected:
  ...
  ClientBase(const std::string &host_port, unsigned short default_port) noexcept
      : default_port(default_port), handler_runner(new ScopeRunner()) {
  ...
  std::shared_ptr<Connection> get_connection() noexcept {
    ...
    if (!query) {
      if (config.proxy_server.empty())
        ...
        query =
            std::unique_ptr<asio::ip::tcp::resolver::query>(new asio::ip::tcp::resolver::query(proxy_host_port.first,
                                                                                               std::to_string(
                           ...
                           [this, session](const error_code &ec, std::size_t bytes_transferred) {
                             ...
                             if (!ec) {
                               ...
                               if (header_it != session->response->header.end()) {
                                 ...
                                 auto chunks_streambuf =
                                     std::make_shared<asio::streambuf>(this->config.max_response_streambuf_size);
                                ...
                                if (!ec) {
                                  ...
                                  asio::async_connect(*session->connection->socket,
                                                      ...
                                                      [this, session, resolver](const error_code &ec,
                                                        ...
                                                        session->connection->cancel_timeout();
                                                        auto
                                                            lock = session->connection->handler_runner->continue_lock();
                                                        if (!lock)
                                                          return;
                                                        if (!ec) {
