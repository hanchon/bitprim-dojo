...
std::ostream &operator<<(std::ostream &output, const encoding &argument) {
  ...
  case encoding_engine::info:value = encoding_info;
    break;
  case encoding_engine::json:value = encoding_json;
    break;
  case encoding_engine::xml:value = encoding_xml;
    break;
  default:BITCOIN_ASSERT_MSG(false, "Unexpected encoding value.");
  }
